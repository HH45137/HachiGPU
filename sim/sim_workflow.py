...
class SimWorkflow:
    ...
    def compile(self, top_module, src_files, output_name):
        ...
        cmd = [self.tools['iverilog'], '-o', f"{output_name}.vvp"]
    ...
    def run(self):
        ...
        src_files = [
            str(self.rtl_dir / f"{base_name}/{tb_name}.v"),
            str(self.rtl_dir / f"{base_name}/{v_name}.v")
        ]
        vvp_name = tb_name
        vcd_name = f"{v_name}.vcd"
        
        if not self.compile(v_name, src_files, vvp_name):
            return False
...
def main():
    ...
    if args.module != None:
        ...
        print(f"Module {args.module} not implementation")
